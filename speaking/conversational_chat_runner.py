...
def count_tokens_from_messages(messages):
    ...
    for msg in messages:
        ...
        try:
            total_tokens += len(
                encoding.encode(msg.get("role", ""), disallowed_special=())
            )
            total_tokens += len(
                encoding.encode(msg.get("content", ""), disallowed_special=())
            )
...
def generate_ground_truth_translation(config):
    ...
    translated_conversation = [
        translate_conversation(r["conversation"]) for r in dataset
    ]
...
def get_few_shot_examples(df, df_sample_group, config):
    ...
    for i, row in df.iterrows():
        df_sample = df_sample_group[
            df_sample_group["translated_conversation"] != row["translated_conversation"]
        ]
            ...
            l_inputs.append(
                {
                    ...
                    "messages": row_translation_prompt
                    + list(row["translated_conversation"]),
                }
            )
...
def generate_prompted_translation(config):
    ...
    for i, row in ground_truth_translation.iterrows():
        ...
        if config["experiment"]["experiment_params"].get("n_few_shot_examples", 0):
            row_translation_prompt += "\n" + row["few_shot_examples"]
        row_translation_prompt = [{"role": "system", "content": row_translation_prompt}]

        l_inputs.append(
            {
                "reference_conversation": row["reference_conversation"],
                "translated_conversation": row_translation_prompt
                + list(row["translated_conversation"]),
                "prompt": row_translation_prompt
                + build_input(row["translated_conversation"]),
            }
        )
    ...
    for i, row in enumerate(l_inputs):
        l_logprobs_prompts.append(row["translated_conversation"])
    logprobs = llm.chat(
    ...
    df_output.to_parquet(
        os.path.join("output", experiment_hash, "data", "prompted_chat.parquet")
    )
...
def judge_cot_style_adherence(config):
    ...
    generated_cot_path = os.path.join(
        "output", experiment_hash, "data", "prompted_chat.parquet"
    )
    ...
    l_judge_prompts = []
    for (_, generated_cot_row), (_, sft_row) in zip(
        df_generated_responses.iterrows(), df_sft.iterrows()
    ):
...
def judge_cot_encoding_English_coherence(config):
    ...
    l_inverted_response = [
        [fn_inverse_encoding_scheme(response) for response in responses]
